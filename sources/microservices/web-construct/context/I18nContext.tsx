...
export function I18nProvider({ bundle, children }: { bundle: I18nBundle; children: React.ReactNode }) {
  ...
  const value = useMemo<I18nContextValue>(() => {
    const t = createTranslator({
      ...
      onMissing: bundle.isDev ? key => console.warn(`[i18n] missing translation: ${key}`) : undefined,
    })
    ...
    return {
      ...
      switching,
      /**
       * Persist the choice, then re-render the RSC tree. `router.refresh()`
       * keeps client component state — open dialogs, half-filled forms, grid
       * filters and selections all survive the switch (§9.4), which a full
       * navigation or `location.reload()` would destroy.
       */
