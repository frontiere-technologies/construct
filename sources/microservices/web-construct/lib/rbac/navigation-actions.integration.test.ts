...
describeIntegration('navigation mutations against the database', () => {
  afterEach(async () => {
    await db.delete(navigationItem).where(like(navigationItem.name, `${PREFIX}%`))
  })
  ...
  it('rolls back item creation when tag replacement fails', async () => {
    ...
    expect(await db.select().from(navigationItem).where(eq(navigationItem.name, name))).toHaveLength(0)
  })
  ...
  it('rolls back field and parent changes when an update tag write fails', async () => {
    ...
    const [row] = await db.select().from(navigationItem).where(eq(navigationItem.idItem, id))
    expect(row.name).toBe(original)
    expect(row.idItemParent).toBe(0)
  })
  ...
  it('serializes concurrent reorders into unique deterministic positions', async () => {
    ...
    const rows = await db.select({ position: navigationItem.orderPosition })
      .from(navigationItem)
      .where(like(navigationItem.name, `${PREFIX}%`))
  ...
  it('serializes subtree deletion with every other navigation write', async () => {
    ...
    try {
      ...
      await deleting
      expect(await db.select().from(navigationItem).where(eq(navigationItem.idItem, id))).toHaveLength(0)
    } finally {
      connection.release()
