...
|---|---|---|
| **1 — Fondazione dati** (questo piano) | rinomina, `menu_entry`, `role_permission`, percorsi di lettura e scrittura | da fare |
| 2 — Catalogo ed enforcement | `permission-catalog.ts`, sincronizzazione, `resolveActor`, `requirePermission`, guardie AST, HOLE-1..4, token ridotto all'identità | da pianificare |
| 3 — Interfaccia | `/menu`, campo *Permesso richiesto*, pagina `/permissions`, semplificazione di `/roles-permissions`, `use-auth` | da pianificare |
