...
- [ ] ID=BUG-5, Severity=Medium, Complexity=Low, Priority=P0, Estimate=minutes, Title=Da confermare in browser, se la revoca di una singola foglia funzioni, Fix description=La segnalazione dice «la deselezione in generale non funziona». Il percorso delle cartelle è dimostrato rotto da BUG-3 leggendo il codice. Il percorso delle **foglie**, letto riga per riga, risulta invece corretto: `applyToggle` scrive `false`, `computeDeltas` produce il delta, `apply_role_permission_deltas` cancella la riga. Prima di riscrivere qualunque cosa va verificato in browser su un ruolo non di sistema: se anche quel percorso è rotto esiste una quinta causa che questa analisi non ha trovato, e va trovata prima e non dopo la riscrittura. È il primo passo del piano, non l'ultimo.
