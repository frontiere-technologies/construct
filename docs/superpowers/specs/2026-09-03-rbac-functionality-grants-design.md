...
- [✅] ID=MIG-5, Severity=n/a, Complexity=Low, Priority=P0, Estimate=minutes, Title=Riduci `permission` ai soli permessi del codice, Fix description=Cancella ogni riga che non sia `operations` né un suo discendente. Il criterio è **strutturale** — discesa di `id_parent` a partire da `operations` — e non un elenco di identificativi noti. Attenzione a **non** descriverlo come «lo stesso criterio di `buildAuthTree`», che è ciò che la prima stesura di questa voce diceva e che la revisione del 2026-09-03 ha accertato falso: `buildAuthTree` parte da *ogni* riga con `id_parent` nullo e **conserva tutte le radici** — il suo test lo asserisce, con `root` (0) e `operations` (−1) come due radici dello stesso albero. Qui la discesa parte da una radice **nota**, e ogni altra radice viene cancellata, `root` compresa. Le due regole divergono esattamente sul punto che conta, e l'errore vive anche nell'intestazione della migrazione `0027`, che è applicata e immutabile: la correzione sta qui: un elenco scritto a mano sarebbe giusto solo sul database di sviluppo. `role_permission.id_permission` è `on delete cascade`, quindi le eventuali concessioni residue su quelle righe se ne vanno con loro — e dopo MIG-3 non ce ne sono.
