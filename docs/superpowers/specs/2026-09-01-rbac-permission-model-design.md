...
- [x] ✅ ID=DEC-16, Title=La tipologia di una voce non si converte, ma i sottotipi di funzionalità sì — decisa il 2026-09-02, dopo che la revisione finale del ramo ha trovato il buco: `updateNavigationItem` scriveva `id_functionality_type` sulla voce senza mai creare il permesso, quindi una categoria convertita in funzionalità restava con `id_permission` nullo — una **voce pubblica** (`isEntryVisible` la mostra a chiunque sia autenticato) e **ingovernabile** (senza riga in `permission` non compare in Ruoli & Permessi da nessuna parte). Si è scelto di **vietare la conversione** invece di implementarla: implementarla bene è lavoro della Fase 3. Il divieto è simmetrico e dedotto dai **dati salvati**, non dall'input, ed è imposto in entrambe le direzioni e su **entrambi i percorsi di scrittura** — la conversione in `updateNavigationItem` e la coerenza della coppia `(idItemType, idFunctionalityType)` in `createNavigationItem`, che ne era la porta di servizio. Il confine difeso è però **uno solo**: fra i tre sottotipi di funzionalità (embedded, link esterno, link interno) il permesso non si muove, il server accetta il cambio, e l'interfaccia lo consente — in modifica la tendina non si disabilita, si **restringe al proprio lato del confine**. Su una categoria quel lato ha una sola opzione, e là il controllo è bloccato con il motivo scritto nel tooltip. La prima stesura disabilitava l'intera tendina e portava via con sé una cosa che funzionava: cambiare un link interno in esterno richiedeva cancellare e ricreare la voce, perdendone id, tag e posizione nell'albero. **Correzione che vive qui e non nella migrazione:** l'intestazione di `0023_functionality_type_locked_hint.sql` — e la sua copia in `schema.sql`, che è generato da quel file — dice che «la tendina è disabilitata in modifica (`typeLocked = mode === 'edit'`)». Era vero quando quella migrazione è stata scritta e non lo è più da quattro commit dopo, nello stesso lotto. La migrazione non si corregge: è già applicata, e `assertAppliedMigrationChecksums` (`migration-lib.mjs`, chiamata da `applyPendingMigrations`, quindi al momento di `db.mjs apply`) rifiuta una migrazione applicata i cui byte cambiano, su **ogni database la cui riga di cronologia ha `completed_at` valorizzato**. È la stessa immutabilità che protegge ogni altro database da una riscrittura silenziosa, e vale anche quando la riscrittura sarebbe innocua. Attenzione a dove sta il controllo, perché è controintuitivo: `npm run test:migrations` **non** lo esercita sui file veri — gira su cartelle temporanee costruite con `mkdtempSync` e non legge mai `sources/devops/db/migrations/`. Chi modificasse la 0023 e lanciasse quel comando vedrebbe verde, e scoprirebbe il problema solo applicando lo schema a un database che l'ha già eseguita. Anche `schema-check` tace, purché lo snapshot venga rigenerato. Vale lo stesso per la `description` della chiave seminata («the type is fixed at creation»), che sovrastima: la tipologia **è** modificabile dopo la creazione, fra i sottotipi. Il tooltip che quella chiave rende, però, compare solo sulle categorie, e là il testo è esatto. Chi tocca di nuovo questo spazio dei nomi metta la correzione nell'intestazione della propria migrazione.
...
- [✅] ID=HOLE-5, Severity=Medium, Complexity=Low, Priority=P2, Estimate=minutes, Title=Concedere una foglia concede gli antenati, revocarla non li revoca, Fix description=In `lib/rbac/permission-tree.ts` `applyToggle` risale gli antenati quando accende una funzionalità e li marca concessi, ma il ramo che la spegne non risale affatto: gli antenati restano concessi per sempre, anche dopo la revoca dell'ultimo discendente. Da qui nascono i permessi residui sulle categorie — sul database di sviluppo, «Home» concessa a due ruoli senza un solo discendente concesso. Trovato il 2026-09-02 durante la revisione del Task 4 della Fase 1. Chiuso il 2026-09-02 dal Task 6 della Fase 1: `applyToggle` non concede più le categorie (causa) e la migrazione `0020` ha eliminato le concessioni residue (dato: 4 righe sul database di test, tutte del ruolo Administrator). Verificato che nessun ruolo perde la colonna «ha permessi» nella lista: entrambi i ruoli con concessioni ne conservano su foglie.
