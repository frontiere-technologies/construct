...
- [✅] ID=BTN-3, Severity=Low, Complexity=Low, Priority=P2, Title=Imbottitura del bottone secondario, Fix description=`px-3` in tredici punti d'uso, `px-4` in tre. Scegliere un valore unico per la variante, così che primario e secondario nella stessa finestra siano proporzionati. **Deciso e chiuso**: la dimensione `default` di `Button` è `px-4 py-2`, condivisa da `default` e `outline`. **Riaperto e richiuso nella revisione finale sull'intero ramo**: `GridToolbarResetButton` e `ColumnVisibilityToggle` restavano al size default (`px-4`) mentre `RolesTableClient.tsx`, `LanguagesTableClient.tsx` e `TranslationsTableClient.tsx` passano `size="sm"` (`px-3`) al primario della stessa riga — la condivisione della dimensione di *default* fra le varianti della primitiva non impedisce a un punto d'uso di scegliere una `size` diversa solo per il primario, che è esattamente cosa succedeva. Corretto rendendo esplicito `size="sm"` anche sulle due secondarie del toolbar, così la riga converge su `px-3` ovunque; `FunctionalitiesTreeClient.tsx:81,102` lo faceva già ed è il modello seguito.
