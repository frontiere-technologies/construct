...
- [ ] ID=ENV-7, Severity=Low, Complexity=Low, Priority=P3, Title=Eliminare il sourcing manuale dell'env nei test di integrazione, Fix description=Caricare l'env dentro `vitest.integration.config.ts` invece di richiedere `set -a && . ./.env.local && set +a`, e semplificare il comando documentato in `CLAUDE.md`.
