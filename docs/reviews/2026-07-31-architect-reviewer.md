...
Remediation verified 2026-08-02: migration `0001_baseline.sql` reads and counts legacy administrators, backfills role `1`, verifies the migrated count, and only then drops `users.role`. A disposable PostgreSQL 17 upgrade fixture preserved its legacy administrator, removed the column, recorded one completed migration, and made the second apply a no-op. The recovery runbook adds preflight, `pg_dump`, repair, verification, rollback, and isolated-restore instructions.
...
Remediation verified 2026-08-02: migration `0002_runtime_boundary.sql` creates a NOLOGIN/NOSUPERUSER/NOBYPASSRLS `construct_runtime` role, role-specific RLS policies, exact DML/sequence/function grants, Data API revocations, an invoker-rights view, and no migration-history access. The operator-only provisioning command created a limited LOGIN member that read application data but could not read migration history, and failed closed for existing logins with unexpected membership, ownership, object grants, direct database/schema ACLs, or default ACLs. Catalog/privilege contracts passed with simulated Data API roles. Operator variables live only in a separate devops template; `MIGRATION_DATABASE_URL` is absent from web environment templates and runtime manifests.
...
Remediation verified 2026-08-02: all Users, Functionalities, Roles & Permissions, Theme, Languages, and Translations pages now live under one URL-transparent `(admin)` route group whose Server Component layout calls the live database-backed `requireAdmin()` guard. Proxy redirects and write-side guards remain in place as separate layers. The focused rejection/success tests passed, and a production build confirmed every public route is unchanged.
...
Remediation verified 2026-08-02: `consume_password_set_token` locks the token row, validates its lifecycle, updates the password, and invalidates siblings within one PostgreSQL statement transaction. The real-database concurrency test proved that two simultaneous claims produce exactly one `ok`, one `used`, and one winning password hash.
...
Remediation verified 2026-08-02: every navigation write—including subtree validation/deletion—uses one Drizzle transaction plus the same transaction-scoped advisory lock for item, tag, parent, and sibling-order changes. Four PostgreSQL integration tests passed, covering atomic rollback, consistent reordering, concurrent mutation serialization, and deletion waiting on the shared lock.
...
Remediation verified 2026-08-02: guarded role/status functions take a shared transaction advisory lock, mutate, and reject the transaction with stable `last_active_administrator` if the post-condition has no active admin. The application retains fresh `requireAdmin()` and self-edit checks but delegates the authoritative invariant to PostgreSQL. Two consecutive concurrent test runs each allowed exactly one competing mutation and retained one active administrator.
...
Remediation verified 2026-08-02: ordered immutable migrations now carry SHA-256 checksums and transactional started/completed history; one session advisory lock serializes competing runners, incomplete identical migrations are retryable, and completed drift is rejected. Two simultaneous PostgreSQL 17 runners produced one complete five-migration history and one no-op. `schema.sql` is generated; the catalog gate compares every modeled table/column type/nullability/default presence, full manual-index structure (columns/order/uniqueness/predicate), foreign key, and view column/type shape to Drizzle, with a deliberately mismatched failing fixture. Six migration-library tests, schema/CI gates, and deployment/recovery runbooks cover drift, backup, rollback, restore, and forward-fix handling.
...
Remediation verified 2026-08-02: registration and the freshly authorized admin resend route share a transactional invitation service. Passwordless duplicates receive a new pending attempt; failed delivery records a bounded error without invalidating an older delivered link; only a successful send supersedes older unused invitations. Four lifecycle integration tests passed, including atomic user/token rollback, retry, password-bearing no-op, failure preservation, success supersession, and rejection of superseded tokens.
...
Remediation verified 2026-08-02: registration, forgot-password, set-password, and credentials authorization all call the shared database-backed limiter with independent keyed-HMAC IP/account buckets and bounded route-specific limits. Four unit tests and the PostgreSQL bucket integration test passed; raw IPs, emails, and tokens are not stored.
...
Remediation verified 2026-08-02: the reusable Kustomize base and dev/production-example overlays now cover immutable production image digest, TLS, two replicas, rolling update, PodDisruptionBudget, non-root/read-only/drop-capabilities security, restricted egress, runtime-only secrets, and dedicated database-bounded readiness plus process liveness. Cookie security follows the external URL scheme and fails safe when production configuration is missing. Nine health/cookie tests, both rendered-overlay contracts, shell syntax, and the production build passed; the runbook covers backup, migration, rollout/rollback, restore approval, and credential rotation.
...
Remediation verified 2026-08-02: the implementation and i18n design now explicitly define these records as best-effort structured diagnostics with no completeness/compliance guarantee. The production runbook assigns stdout routing, access, retention, monitoring, and redaction to the derived deployment. Both documentation contract tests passed and prevent reintroducing durable/append-only claims.
...
Remediation verified 2026-08-02: `app_language` now drives both UI language selection and the navigation authoring locale list. Navigation keeps its compatible JSON/tag storage, normalizes configured codes to uppercase keys, preserves inactive authored languages, validates required fields against the configured default, and resolves labels through active → configured default → English → database name. Forty focused locale/sidebar/tree/component tests passed and the production build retained every route.
...
Remediation verified 2026-08-02: both reverse lookup indexes exist in SQL and Drizzle. Forced-plan `EXPLAIN` selected an index-only scan for `user_role(id_role, user_id)` and an index scan for `navigation_item(id_item_parent, order_position)`. README keeps the simple full read until measured sidebar p95/query cost breaches the derived application's database budget, at which point traces—not an arbitrary row count—drive cache/invalidation work.
