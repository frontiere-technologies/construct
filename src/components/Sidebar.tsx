...
import { MenuItem, MenuPosition } from '../types/menu';
...
// ── Fix #1: ColToggle defined outside Sidebar to avoid remount on every render ──
...
// ── Fix #2: L1Item as a proper React component with key managed by the caller ──
...
// ── Fix #2: SubItem as a proper React component ──
// Fix #6: hover props passed explicitly to NavLink instead of spread
...
const SubItem: React.FC<SubItemProps> = ({
  ...
  // Fix #6: explicit props on NavLink instead of spread
  if (item.route) {
    return (
      <NavLink to={item.route} onMouseEnter={tooltipEnter} onMouseLeave={tooltipLeave} className={cls}>
        {icon}{label}
      </NavLink>
    );
  }
...
// Fix #5: helper to read persisted collapse state
...
export const Sidebar: React.FC = () => {
  const { menuItems, menuLoading, settings, setSettings } = useMenu();
  ...
  // Fix #5: collapse state initialised from localStorage
  ...
  // Fix #5: persist collapse preference on change
  useEffect(() => {
    try {
  ...
  // Fix #3: tooltip rendered via createPortal — stable callbacks to avoid child re-renders
  ...
  // Fix #4: effect depends on menuItems (state ref changes only on actual updates)
  useEffect(() => {
    const active = menuItems.find(i => i.route === location.pathname);
    if (!active) return;
  ...
  // Single active item by route — only ONE item highlighted per URL, not all with same route
  const activeRouteId = menuItems.find(i => i.type === 'link' && i.route === location.pathname)?.id ?? null;
  ...
  }, [menuItems, selectedL1Id, navigate]);

  // Fix #7: handleL2Click navigates direct links when container has no visible children
  ...
  return (
    <div className="flex h-screen flex-shrink-0">
      ...
      {/* Fix #3: tooltip via portal — escapes transform/filter/overflow ancestors */}
      ...
      <aside className={clsx(
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-2 space-y-1 scrollbar-hide">
          {menuLoading
            ? [...Array(5)].map((_, i) => (
                <div key={i} className="h-10 rounded-lg bg-sidebar-text/10 animate-pulse" />
              ))
            : mainItems.map(item => (
                <L1Item key={item.id} item={item} isSelected={selectedL1Id === item.id}
                  isActive={selectedL1Id === null && item.id === activeRouteId}
