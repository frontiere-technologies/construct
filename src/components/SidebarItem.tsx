...
export const SidebarItem: React.FC<SidebarItemProps> = ({ item, level, childrenItems, allMenuItems }) => {
  ...
  const isChildActive = (parentId: string): boolean => {
    ...
    return children.some(child => 
      child.route === location.pathname || isChildActive(child.id)
    );
  };

  const isActive = item.route === location.pathname || (hasChildren && isChildActive(item.id));
  ...
  return (
    <div className="mb-1">
      {item.type === 'link' && item.route ? (
        <NavLink to={item.route} className={itemClasses} style={{ paddingLeft: `${paddingLeft}px` }}>
          {renderContent()}
        </NavLink>
      ) : (
